...
(() => {
  try {
    ...
    const pluginFiles = readdirSync(sourceDir)
      .filter((file) => file.endsWith(".ts") && file !== "index.ts");

    const pluginImports = pluginFiles
      .map((file) => `import plugin_${file.replace(/\.ts$/, "")} from './${file.replace(/\.ts$/, "")}';`)
      .join("\n");

    const pluginArray = pluginFiles
      .map((file) => `plugin_${file.replace(/\.ts$/, "")}`)
      .join(", ");
